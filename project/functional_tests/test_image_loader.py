...
TEST_IMAGE_RESIZED_NAME = ('2e2d97e2ce9059a06b285c5d7293d1a8a63ee836_resized'
                           '.jpg')
...
class ImageLoaderTest(LiveServerTestCase):
    ...
    @remove_image_after_test([TEST_IMAGE_NAME, TEST_IMAGE_RESIZED_NAME])
    def test_loader_image(self) -> None:
        ...
        upload_image_list = self.browser \
            .find_element_by_id('uploaded_image_list')
        ...
        upload_image_file_input = self.browser \
            .find_element_by_id('upload_image_file_input')
        upload_image_file_input.send_keys(
        ...
        self.browser.find_element_by_id('change_width_image_input') \
            .send_keys('400')
        ...
        upload_image_list = self.browser \
            .find_element_by_id('uploaded_image_list')
        self.assertEqual(
            upload_image_list.text,
            settings.IMAGE_UPLOAD_FOLDER + '/' +
            TEST_IMAGE_NAME
        )

    @remove_image_after_test([TEST_LINK_IMAGE_NAME])
    def test_loader_image_using_link(self) -> None:
        ...
        upload_image_link_input = self.browser \
            .find_element_by_id('upload_image_link_input')
        upload_image_link_input.send_keys(
            TEST_LINK_IMAGE_PATH +
            TEST_LINK_IMAGE_NAME
        )
        self.browser.find_element_by_id('upload_image_form_submit').click()

        # Пользователь убедился что картинка присутствует на странице
        uploaded_image = self.browser.find_element_by_id('uploaded_image')
        self.assertIn(
            TEST_LINK_IMAGE_NAME,
            uploaded_image.get_attribute('src')
        )
